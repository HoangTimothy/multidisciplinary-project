...
Các tham số quan trọng được gom vào \texttt{config.py}, gồm đường dẫn model, URL tải model EfficientDet-Lite0 int8, kích thước frame, chu kỳ suy luận, ngưỡng confidence, số detection tối đa, ngưỡng gần--xa, vùng trung tâm, cooldown giọng nói và nhãn tiếng Việt. Sau benchmark laptop trên subset COCO+BDD, cấu hình hiện tại dùng \texttt{INFER\_EVERY\_N\_FRAMES = 5}, \texttt{SCORE\_THRESHOLD = 0.4} và \texttt{MAX\_RESULTS = 10} để tăng recall nhưng vẫn giữ model int8 nhẹ cho Raspberry Pi. Việc gom cấu hình giúp nhóm dễ tinh chỉnh khi chuyển từ laptop sang Raspberry Pi hoặc khi đổi camera.

Với benchmark, cấu hình được tách thành JSON để mô tả một experiment đầy đủ: model, threshold, số detection, kích thước frame, chu kỳ inference, queue size, ngưỡng gần--xa, vùng trung tâm, trọng số distance/center/direction và cooldown âm thanh. Cách này giúp chạy lại cùng một cấu hình trên laptop hoặc Raspberry Pi và so sánh kết quả bằng cùng một format.
...
Benchmark offline trên laptop đã có runner, dataset public subset và policy chọn model. Kết quả này đủ để chọn production candidate laptop-first, nhưng vẫn cần nghiệm thu runtime trên Raspberry Pi/ESP32-CAM trước khi gọi là cấu hình production phần cứng.
\begin{table}[H]
  \centering
  ...
  \begin{tabularx}{\textwidth}{L{4.0cm}Y L{3.0cm}}
    ...
    Độ trễ suy luận & Thời gian suy luận offline trên laptop theo \texttt{benchmark\_inference.py} & Winner \texttt{tuned\_lite0\_int8\_high\_recall}: p95 31.4 ms, avg 28.1 ms trên 500 ảnh \\
